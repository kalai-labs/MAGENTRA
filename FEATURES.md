...
- [ ] **Reasoning is shown inside the output** — reasoning stays counted in the output total — it is billed as output — and is now shown as its own part of it: "out 200k · reasoning 135k". The OpenAI-compatible provider reads the reasoning count a provider reports (`completion_tokens_details.reasoning_tokens`); where no provider reports it, the live counter's own split of the stream (reasoning characters vs answer characters) stands in, marked as estimated. The desktop meter, the TUI and `/session` show it. `pure` + `net` + `fs` + `ui` + `llm`
- [ ] **Every tool call has a start and an end** — a tool call the engine refuses before it can run — a switched-off tool, an unknown tool, arguments cut off mid-JSON, input that fails the schema — still sends `tool_call_started` before its `tool_call_finished`, so every frontend draws a row for it and pairs the two. `fs`
- [ ] **Long silent work stays visible** — a long stretch of reasoning never looks like a frozen window, and never goes on unannounced. The live reasoning block's summary counts time and size ("reasoning · 8m12s · ~12k tokens") and the now-line's timer reads in minutes; and once the model has reasoned at length through tool rounds without a word to the user, its next request asks for one sentence about what it is doing — once per silent stretch. `fs` + `ui`
...
- [ ] **Guard status lines tell the truth** — every line that tells the user what the deletion guard and OVERDRIVE do — the engine's status line when the guard is switched, the `/overdrive` lines, the desktop footer hint and its glossary — says what the permission engine really does in that stance: OVERDRIVE turns the deletion guard off, and outside it commands run while deletions ask. `proc` + `ui`
...
- [ ] **The sessions list follows the live session** — the Sessions list is asked for again when a turn starts — the first message is what puts a session on disk — and when it ends, by the tab the turn belongs to, so the live session is listed during its first turn and a background tab's list is its own. `ui`
- [ ] **The console reads plainly** — what the console shows can be told apart and read in full: reasoning stays collapsed by default and apart from the tool rows and answers around it; a provider retry is announced on the now-line; one long unbroken line never gives the transcript a sideways scroll bar; and the model picker names a model by its own name, with the full id on hover, never as "Custom…" over a truncated path. `ui`
