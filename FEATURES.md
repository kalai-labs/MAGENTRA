...
- [x] **Two main-loop rounds, not four** — the proposal format rides in the scout's own system prompt, so its first response with no tool call in it IS the proposal. The phase used to spend two further rounds after the reading was already finished: one announcing it was ready, and one "review your own draft" against a draft that did not exist yet. Recognized by counting H1 headings, which works in any language; a scout that stops without proposing is prompted once, explicitly. `pure` + `llm`
- [x] **Review pass, folded in** — the three questions (right request? unasked-for scope? a guess about this repository?) run inside the same inference that writes the proposal, against the real draft. Leaving it unchanged is a correct outcome, and it reads nothing new. `llm`
- [x] **Grounding floor** — the engine knows which source files retrieval ranked and which the scout opened, so a proposal about behaviour written from the README alone is caught with no model involved. One reminder naming the ranked files, then never again; it does not block, because a documentation request genuinely needs no code. Resets on a revision, which lands somewhere else. `pure` + `fs`
...
- [x] **A cut-off question round still asks** — the reply is long, so the way this layer fails is truncation, and strict JSON parsing read a correct-but-cut-off answer as "ask the user nothing", silently. Three defences, because a token budget is a guess: a budget sized against the measured worst case; salvage of the complete question objects from the unterminated array (brace balancing that respects strings and escapes); and `runInference` reporting its stop reason, so the engine KNOWS the reply was cut off and asks once more, shorter, when salvage recovers nothing. A well-formed empty answer is a decision and is never retried. The same undersized budget and silent outcome existed in the clarify pre-layer and were fixed there too. Each pre-layer also announces itself BEFORE its inference, so the wait is never a blank screen. See ADR 0008. `pure` + `llm`
- [x] **One language, decided from the user's own words** — the request is quoted verbatim into both the question prompt and the proposal prompt, and the language is chosen from that quote alone. "Write in the user's language" on its own drifts to whatever the model has just been reading, which in a repository whose comments are in another language produces headings in one and a body in the other; no English exemplar sentence is offered anywhere for copying. The five headings are translated naturally with order, count and meaning preserved — nothing parses them, so the translation is safe. `pure` + `llm`
