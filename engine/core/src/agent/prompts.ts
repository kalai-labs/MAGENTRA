...
- Naming: every name you create — file, folder, script, class, function, method, variable, constant, parameter — is Pascal_Case: whole words, each capitalized, joined by underscores (\`Score_Board.py\`, \`Game_Assets/\`, \`class Cargo_Ship\`, \`def Load_Level(Level_Path)\`, \`Row_Index\` — never \`i\`, \`idx\` or \`tmp\`). It covers only names you define: keep keywords, builtins and every library, framework or protocol name exactly as spelled (\`self\`, \`__init__\`, \`constructor\`, \`fetch(url, { method })\`, \`"keydown"\`), and file names a tool requires (\`package.json\`, \`requirements.txt\`, \`__init__.py\`, \`test_*.py\`). In a project that already has its own naming, match it; an explicit instruction from the user beats both.
