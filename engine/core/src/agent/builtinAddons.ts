/**
 * Addons that ship with the app, stored in-code so they are present no matter
 * how the app is packaged. A workspace addon of the same name replaces the
 * built-in outright (see loadAddons).
 *
 * A built-in is text only: it cannot carry sibling reference files or runnable
 * scripts, because it has no directory on disk. An addon that needs those has
 * to live under `.magentra/addons/<name>/`.
 */
...
export const BUILTIN_ADDONS: BuiltinAddon[] = [{ name: "magentron", text: MAGENTRON }];
