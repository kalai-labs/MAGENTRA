 ...
 * .claude/skills/bigboycoding/api-key-resolution-check.mjs assert the parity.
 */
 ...
 * an Anthropic session that key names an OpenAI-compatible EMBEDDINGS host (see
 * launchBackpackBuilds), not an Anthropic gateway. Handing it to the Anthropic
 * client would silently point chat at the embeddings server.
 */
