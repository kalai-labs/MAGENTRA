...
|---|---|---|---|---|
| T00 | — | Preparation: baseline and skill doc | done | 2026-09-23. Commit: a8c9150 (committed by the owner after the session). Owner decision: leave the 18 stale gateway records alone, only keep the list below (later tasks do not re-record them). Baseline recorded under this table. `bigboycoding` SKILL.md: rewrote the verification section (build first, the kind → command table, count before/after, revert-verify, gateway first + freshness, approved artifacts, smoke and CI). Replaced the deleted `*-check.mjs` "spec for the rebuild" with a coverage map checked against `tests/features/` and a 4-row backlog of what nothing guards yet (TUI cell layout, no-reflow, folder trust, compaction sizing). Also fixed the other false test-setup lines (intro "no test suite", "`dist/` is committed", "no test catches a system-prompt regression"). **Args bug: real.** Claude Code replaced the literal `$ARGUMENTS` in the addon-check row with the caller's args. I removed the token and added a gotcha. Re-invoked with a probe string: it now appears only at the end, as `ARGUMENTS: …`. No product code or tests changed. After the edit, the same commands gave the same counts (`npm test` 505 passed / 0 failed / 85 withheld, `test:ui` 51 / 0, typecheck clean). Left for T14: the "system map" section (`docs/big-picture/` does not exist). The file counts (75/24/34) are still correct. **Files changed:** `.claude/skills/bigboycoding/SKILL.md`, `FIX-PLAN.md`. |
| T01 | U-01, U-09, R-1 | Reasoning stream does not freeze the renderer | done | 2026-09-23. Commit: a8c9150 (committed by the owner after the session). **Gateway:** new record `long-streams-never-stall-the-window` (ui, deferred by rule), 6 tests; description `a62978d8…` approved by the owner in chat, still `draft` on disk (the draft→ready click is the owner's). **Owner decisions:** approve the record/invariant/checklist as written; a LIVE reasoning block shows its tail, all text when it ends; the records this change makes stale go on the stale list, not reconciled. **Cause, measured on the real app through the real IPC:** with the reasoning block OPEN, 2,000 deltas took 6.7 s at the start and 109.9 s after 10,000 (quadratic), and a task update sent after 12,000 reached the rail 301 s late; closed it was linear (~0.25 ms/delta) but grew one DOM node per delta. An answer that is one long block with no committable blank line was quadratic too (3.2 s → 28.7 s per 2,000): a forced layout per delta, plus `markdownCommitPoint` re-counting the whole message once per blank line inside an open fence. **Fix (renderer only):** reasoning deltas queue on their own block and reach the page once per animation frame (a 250 ms timer covers a window that draws no frames); a live block shows its last ~8,000 characters behind a "N earlier characters are held back" line, and all of it is written into the block when it ends; the live edge is measured once per frame, not per delta (`followLiveEdge`); the commit-point scan is fed each delta (incremental, same verdicts); the answer's live tail is appended, not rewritten; the now-line is set once per stretch ("thinking · 45s", "responding · 12s"), never a token, never a per-delta timer reset. **After:** 500 reasoning deltas cost 5–8 ms flat (block open); a task update after 20 s at 250 deltas/s is handled 5–13 ms after it is sent (was 12.8 s); the long code answer's late/early ratio is 0.75 (was 4.9); 36,000 answer deltas in one 171 KB block with no blank line: 119 → 169 ms per 6,000 (was 192 → 1,271 before the append step). **Field log replayed by hand once** (132,035 frames at 20×, all 35 reasoning blocks opened): 335/335 task/tool/turn/question frames handled in order, lag p50 4 ms / p95 28 ms / max 40 ms, live body ≤ 2 DOM nodes. **Not done:** a live run with a real reasoning model, watched on screen (the plan's manual check) — the replay above stands in for it; the owner should try one. **Tests:** `tests/features/long-streams-never-stall-the-window.test.ts` (6, `ui`). Revert-verified: with the fix stashed, tests 1–4 fail with the field symptoms (4.7×, 12.8 s late, "thinking · fjord · 0s", 4.9×); 5–6 are guards and pass on both. Mutation-checked: tail never trimmed → test 1; held-back text lost → tests 1+5; one global buffer → test 5; token on the now-line → test 3; per-delta scroll → test 4 (4.4×); whole-message commit scan → test 4 (3.1×). NOT caught by a test: rewriting the whole live tail per delta (it only shows at ~171 KB; covered by measurement). **Gates:** build exit 0; `npm test` 596 registered, 504 passed, 1 failed, 91 withheld (85 + the 6 new ui) — the red is `import-graph · an-edited-file-is-re-extracted…`, a pre-existing flake (1 of 10 runs with AND without this change: it asserts an mtime moved when two writes can land in one tick); `npm run test:ui` 57 passed, 0 failed; `typecheck:tests` clean; `smoke` exit 0. **System-wide:** engine and protocol unchanged; the TUI needs nothing (it paints at ~30 fps and never draws reasoning, `useEngine.ts:15-22, 462-468`); per-tab: pending text lives on its element, so a background tab's reasoning lands in its own pane (test 5, tiled + a real focus change); session replay still shows the whole reasoning (test 6); a reload drops only what the page held; no IPC batching in main — not needed at the measured cost. **Found, not fixed:** (1) `onToolOutputDelta` still does a forced layout per delta (same class of bug, for noisy commands); (2) the log's redaction depth cap turns every `question_request` option into "[depth capped]" and 107 lines are cut invalid (for T05). **Files changed:** `app/renderer/modules/landing.js`, `app/renderer/modules/stream.js`, `app/renderer/modules/util.js`, `app/renderer/styles.css`, `tests/features/long-streams-never-stall-the-window.test.ts` (new), `tests/gateway/features/long-streams-never-stall-the-window.json` (new), `tests/gateway/descriptions/a62978d8-27b4-42a7-b737-0bbeac0ab5a0.json` (new), `FEATURES.md`, `FIX-PLAN.md`. |
| T02 | U-02 | Renderer watchdog and recovery | skipped (owner) | 2026-09-23/24. **Owner decision: skip T02 entirely** ("do not do anything for this since we don't know what window stall or when happens"; the resync option: "skip"; asked again: "Skip T02 entirely"). No code. **Facts found, kept for later:** a renderer reload keeps every engine alive in main (`engineTabs`), but `boot()` always lands on the start page with an empty tab map, and nothing re-attaches; `resume_session` is refused while a turn is busy and replays only finished messages; no frame lets a frontend ask an engine for its live state; `createExtraWindow` wires no `render-process-gone`/`unresponsive` handler at all; after the Home button, re-opening the same workspace sends `tab:focused` for a tab the renderer does not know (ignored), and frames with unknown tab ids run in the focused tab's state (possible cross-tab bleed with several running tabs — unverified). **Files changed:** none. |
| T03 | S-01, S-02 | Process-kill guard and the guard message | done | 2026-09-23. **Owner decisions:** a kill by process name ASKS outside OVERDRIVE and is REFUSED in OVERDRIVE (pointer to TaskStop / a pid); only a literal grant or an explicit rule for that exact command overrides it, in both stances; the "Allow deletions" switch does not touch it; Bash description bullet "To stop a background command, use TaskStop…" approved; UI wording approved; OVERDRIVE prompt sentence, CONTEXT.md and three record invariants updated (allow-all-stance, permission-stances, deletion-guard — its old "always asks, in both stances" contradicted its own OVERDRIVE test). **Classifier** `bashProcessKillSubject` (bash.ts) reads COMMAND POSITIONS through a small quote/substitution-aware tokenizer (so `git commit -m "…pkill…"` is not refused): taskkill /IM or without /PID (`/`, `//`, `-` forms), pkill, killall, tskill <name>, Stop-Process/spps unless by id, PowerShell `kill -Name`, `kill -1`, wmic process delete/terminate, Win32_Process terminate, any kill fed by pgrep/pidof/ps/Get-Process/tasklist; not kills by pid or by port. Wired into Bash AND Monitor. **PermissionEngine:** new block right after deny rules; resolution order is now deny > process-kill > protected-path > deletion > allow > stance; an approved kill still meets the deletion guard (both notes reach the model). Desktop card shows the guard's line (`approvalLines` shared by modal and pane); TUI already showed descriptions. **S-02:** set_deletion_guard line, /overdrive lines, footer hint ("runs commands · deletions ask" / "autonomous · nothing asks" — the old one also said "asks before commands", false since 2026-07-26), glossary. **Records:** `process-kill-guard` (pure+proc+ui, 8 tests), `guard-status-lines-tell-the-truth` (proc+ui, 2 tests; kinds re-declared from fs because the check runs a real shell). **Revert-verified:** guard off → 7/8 fail, live test "the command ran in OVERDRIVE"; old lines → "promises a prompt no stance always makes"; old card/footer → ui fails. **Found, not fixed:** Monitor has no deletion guard (pre-existing); FEATURES.md prose for Deletion guard / Allow-all stance still says "in both stances" (record prose is verbatim identity). **Files changed:** `engine/tools/src/bash.ts`, `engine/tools/src/monitor.ts`, `engine/tools/src/index.ts`, `engine/core/src/agent/tool.ts`, `engine/core/src/runtime/permissions.ts`, `engine/core/src/runtime/session.ts`, `engine/core/src/runtime/engine.ts`, `engine/core/src/state/transcript.ts`, `app/renderer/index.html`, `app/renderer/modules/{dom,landing,tabs,state}.js`, `app/renderer/styles.css`, `CONTEXT.md`, `FEATURES.md`, tests (2 new + allow-all-stance, permission-stances, deletion-guard, prompt-contract), 2 records + 2 descriptions, the allow-all-stance ready description, `tests/lib/uiTest.ts` (waitFor promoted from the T01 test). |
| T04 | M-06, R-3 | Progress comments during long work | done | 2026-09-24. **Options chosen (owner: "I approve the things"):** UI + engine. **Engine:** a silent-reasoning rung — after a model call that ends in tool calls with no user text, its reasoning adds to a per-turn count; at 8,000 characters the next request carries `reminder.silent-reasoning` ("tell the user in one short sentence what you are doing…") once per silent stretch, re-armed by text; root only. **UI:** the live reasoning summary reads "reasoning · 8m12s · ~12k tokens" (reuses estimateTokens/formatTokens/formatElapsed), a restored block "reasoning · ~2.0k tokens"; the now-line timer reads in minutes, one helper (`nowActivityTail`) for the strip and every pane. No per-call reasoning budget (provider-dependent). **Record:** `long-silent-work-stays-visible` (fs+ui, 3 tests). **Revert-verified:** 3/3 fail. **Files changed:** `engine/core/src/runtime/session.ts`, `app/renderer/modules/{stream,views,tabs,landing}.js`, test + record + description, `FEATURES.md`. |
| T05 | L-01…L-04 | Session log: tokens, valid JSON, size, per workspace | done | 2026-09-24. **Multi-workspace misrouting CONFIRMED in code** (one `currentLogFile`; engine frames logged with no tab). `logging.js`: redaction by secret-name endings and words (…key, …token, …secret, x-api-key, secrets, apiKeys, …) plus credential-shaped values (Bearer, sk-…, ghp_…, fw_/hf_/gsk_…), so `*Tokens` survive; long strings/arrays shortened INSIDE the object with a marker ("…[+49000 chars]"), depth cap 4→12, so every line is JSON and question options are logged; `logEngineFrame` folds thinking/text/tool_output deltas per tab (a run ends at another frame, after 1 s, or at a flush) with counts and adds `durationMs` to tool_call_finished; `logEvent(ch, data, target)` routes a tab's lines to its own workspace log and names the tab; file name is UTC with `Z`; `setLogWorkspace` now flushes instead of emptying the queue (it threw away unflushed lines on every focus change — found while reading). main.js: `tabLog(tab)` on kill/spawn/exit/stderr/unparseable/ui/drop lines; renderer state-sync frames before an engine exists log as `state-sync-skipped`. **Record:** `session-log-reads-on-its-own` (fs+ui, 6 tests). **Revert-verified:** 6/6 fail with the field symptoms. **Files changed:** `app/main/logging.js`, `app/main.js`, test + record + description, `FEATURES.md`. |
| T06 | V-01, V-05, R-4 | Verify the result the way the user uses it | done | 2026-09-24. **Why the rung never helped (transcript):** the runtime-evidence rung — the only place that says "capture a screenshot and Read it" when vision is on — fires only when a turn ran NO command; the field turn ran 45 Bash calls, so it fired 0 times ("screenshot" appears 0 times in the transcript). **Fix (instruction-only, no capture tool):** a third shape of the same floor and fuse: a turn that changed a browser-facing file (.html/.css/.scss/.vue/.svelte/.jsx/.tsx…) and has no browser evidence (a Bash command driving playwright/puppeteer/selenium/--headless/--screenshot…, or a successful Read of an image) gets `finishing.browser-evidence` once: drive the page in a real or headless browser, do the user's main goals with default settings, read the result (VISION_ON/OFF reused), check feedback and on-screen rules, or say it stays unverified. **Record:** `web-changes-are-seen-in-a-browser` (pure+fs, 4 tests). **Revert-verified** (2 mutations). **Found, not fixed:** the stand-in shape of runtime-evidence opens with "did not run a single command" although commands ran (pinned by its tests). **Files changed:** `engine/core/src/runtime/{finishing,session}.ts`, test + record + description, `FEATURES.md`. |
| T07 | V-02, V-03, V-04 | Honest "works" claims and a real self-check | done | 2026-09-24. OVERDRIVE only, as before (an attended turn is read by its user). The engine collects the failure sentences the model wrote during tool rounds (`findSymptoms`, ≤6) and the hedges of its final answer (`findHedges`: may still, not verified, unverified, could not verify, should work…, ≤5) and quotes them inside the self-check's one `{{closing}}` slot: `finishing.self-verify.symptoms` (settled only when re-tested after its fix; one passing case does not settle another) and `finishing.self-verify.hedges` (settle it now, or keep it as a plain note; "may still" never stands in for a cleanup). The code closing adds: search every file for a mistake the fix corrected. None demands a pass (honest-gap test still green). **Record:** `the-self-check-names-what-the-turn-left-open` (pure+fs, 3 tests; the fs one replays the field sentences). **Revert-verified.** **Files changed:** `engine/core/src/runtime/{finishing,session}.ts`, test + record + description, `FEATURES.md`. |
| T08 | U-03, U-04, U-05, R-2 | Engine timestamps for tasks and timers | done | 2026-09-24. **Shape (a):** optional `at` (epoch ms) on turn_started / tool_call_started / tool_call_finished; `TaskItem.startedAt` / `completedAt` stamped by TaskStore on status change (pending→completed gets no start; reopening clears the end), persisted, so a resume keeps them. tool_call_finished.at is each call's OWN finish. Desktop: task rail, tool rows and the turn timer use the engine times (fallback: when the frame was seen). TUI: `turn_started.at` mirrored; its turn timer starts from it. wire-round-trip samples extended; tui-protocol-parity green. U-03 (rail at 0/7) was the T01 backlog. **Record:** `durations-come-from-the-engine-clock` (fs+ui, 3 tests). **Revert-verified:** "turn_started carries the engine's time"; ui "0s" vs "1m55s". **Found, not fixed:** a sequential batch's finished frames are emitted after the whole batch, so rows show running until the batch ends. **Files changed:** `engine/protocol/src/types.ts`, `engine/core/src/state/taskStore.ts`, `engine/core/src/runtime/session.ts`, `app/renderer/modules/{tasks,views,landing,stream}.js`, `tui/src/protocol.ts`, `tui/src/engine/useEngine.ts`, `tests/features/wire-round-trip.test.ts`, test + record + description, `FEATURES.md`. |
| T09 | U-06, U-07, U-08, U-09 | UI: reasoning vs progress, sessions, retries, small defects | done | 2026-09-24. **U-06:** reasoning is collapsed by default — no `open`, no CSS forcing it; the field screenshot shows a block opened by hand (guard test). **U-07:** `requestSessionList()` also on turn start (the first message writes the session) and turn end, sent to the tab being dispatched; TUI SessionPicker already asks afresh. **U-08:** retry_status was already on the now-line (strip and pane) — invisible only because of T01 (guard test). **U-09 scroll, reproduced first:** a 600-character command_output/error line gave #transcript scrollWidth 3,753/4,908 vs clientWidth 1,521 (tool rows, code, tables did not overflow — the plan's .tool-row guess was wrong); fix `.stream { overflow-wrap: anywhere }`. **U-09 model label:** last path segment, full id as the option's title, a configured model unknown to the list gets its own option instead of "Custom…" over a truncated box. **Records:** `the-sessions-list-follows-the-live-session` (ui, 2), `the-console-reads-plainly` (ui, 4). **Revert-verified.** **Files changed:** `app/renderer/modules/{landing,session}.js`, `app/renderer/styles.css`, 2 tests + records + descriptions, `FEATURES.md`. |
| T10 | U-10 | Output tokens: show the reasoning part | done | 2026-09-24. Reasoning stays INSIDE output. `Usage.reasoningTokens` / `reasoningEstimated` and `context_update.reasoningTokens` (optional; TUI mirror); `addUsage` carries the part only once reported; `reasoningPart()`. OpenAI-compatible adapter reads `completion_tokens_details.reasoning_tokens` (clamped). Anthropic has no separate count → estimated. The session estimates an unreported part from the streamed reasoning (marked estimated) and splits the live figure. Shown: `/session` "output (reasoning ~2.0k)", desktop strip/pane "↑ 12k out · ~8.1k reasoning", inspector, TUI composer and turn footer. **Fireworks (this folder's connection, deepseek-v4p1-flash, llm test run once):** "reasoning 7 of 10 output tokens — REPORTED by the provider". GLM-5.3 itself not checked. **Record:** `reasoning-is-shown-inside-the-output` (pure+net+fs+ui+llm, 5 tests). **Revert-verified** (4 non-llm). **Files changed:** `engine/protocol/src/{types,tokens}.ts`, `engine/providers/src/openai-compat.ts`, `engine/core/src/runtime/{session,sessionStats}.ts`, `app/renderer/modules/{tokens,session,tabs,views,workbench,landing}.js`, `tui/src/{protocol,format,types}.ts`, `tui/src/engine/useEngine.ts`, `tui/src/components/{Composer,TranscriptLine}.tsx`, test + record + description, `FEATURES.md`. |
| T11 | M-01, M-02, M-04, M-05 | Less silent drafting, short Edit anchors | done (text) | 2026-09-24. Owner approved in advance. SECTION_WORKING_METHOD: "Plan briefly, then write. Never draft a whole file in your reasoning and then write it out again…". Edit description: "Keep old_string short: the smallest unique anchor — a few lines at most — copied from your latest Read…". M-05: the 12-char header is enforced by zod and stated in the schema — unchanged. Reasoning effort: the field run used Think: default; no default changed (a lever left to the owner). **Snapshots fail on purpose until the owner runs `npm run approve`:** system-prompt-is-pinned (1), tool-wire-contract-is-pinned (2, which also carry the T03/T12 Bash bullets). **Files changed:** `engine/core/src/agent/prompts.ts`, `engine/tools/src/edit.ts`. |
| T12 | E-02, E-03 | Tool frame pairing and background-job waits | done | 2026-09-24. **E-02:** one `refuse()` helper for the four early refusals (switched off, unknown, cut-off JSON, schema-invalid) emits tool_call_started before the error, so both frontends draw the row. Scripted provider gains `FakeToolCall.json` (to play a cut-off call). **E-03:** lifecycle pinned FIRST — `tool-bash-background` had no test: id returned before the exit, start/exit notifications, output file, exit code, the task-notification rides the next round. Option (c) chosen: a Bash description bullet — keep waits short and check the job with TaskOutput block:false between tries; (b) rejected (killing a foreground command when an unrelated job dies breaks working builds), (a) is a new feature. **Records:** `every-tool-call-has-a-start-and-an-end` (fs, 1); `tool-bash-background` tests added (proc, 1). **Revert-verified** (E-02). **Files changed:** `engine/core/src/runtime/session.ts`, `engine/providers/src/fake.ts`, `engine/tools/src/bash.ts`, 2 tests + record + description, `FEATURES.md`. |
| T13 | E-05, E-04, E-06 | Edit error hint, shell-edit tracking, stats saved often | done | 2026-09-24. **E-05:** Edit's not-found error adds where old_string stops matching (binary search over prefixes, ~log2(n) scans): both sides quoted at the first difference with the file's line, a CRLF-vs-`\n` mismatch named, "Not even its beginning appears" when almost nothing matches (tool-edit record extended; the field case). **E-04:** `FileState.changedSinceRead()` (checkFresh's test over every Read file; 8–11 ms for 2×500 stats); a foreground Bash result names the Read files the command changed, never ones stale before it (tool-bash/tool-bash-cwd tests now give strictServices a real FileState — the guard named the new dependency). **E-06:** `Session.saveMeta()` is the one writer of the meta record: forced at turn end as before, and after each model call at most every 30 s (root only). **Records:** `a-shell-command-names-the-read-files-it-changed` (proc, 1), `a-running-turn-keeps-its-spend-on-disk` (fs, 1), tool-edit (+1). **Revert-verified** (all three). **Files changed:** `engine/tools/src/{edit,bash}.ts`, `engine/core/src/runtime/{fileState,session}.ts`, `engine/core/src/agent/tool.ts`, tests + records + descriptions, `FEATURES.md`. |
...
### Review round and gates after T03–T13 (2026-09-24)

An independent read-only review of the whole T03–T13 diff ran before the commit. **Fixed from it** (each with a test that fails without the fix):

- **High — kill guard bypass:** a kill behind a shell keyword (`if … then pkill`, `for … do kill $pid`, `while …`, `! pkill`) or `eval` was not seen. `resolveCommand` now skips keywords; `eval` is a wrapper.
- **Medium — kill guard false positives (OVERDRIVE refuses them):** a kill by literal pid beside an unrelated `ps`/`tasklist` (the name-lookup rule now needs a kill whose targets are not all literal pids / `%jobs` / `$!`); here-document bodies (commit and PR messages) read as commands (bodies are skipped unless a shell reads them); `#` comments; backslashes inside double quotes were dropped (`"C:\Windows\…\taskkill.exe"`).
- **Medium — per-tab leak:** a background tab's `session_list` painted the focused tab's sidebar; `onSessionList` now only stores it for that tab (focus repaints it).
- **Medium — refused calls without a start frame:** permission denials (deny rule, declined card, the OVERDRIVE kill refusal) now send `tool_call_started` first. A PreToolUse hook block stays unannounced on purpose — the `hooks` test proves "never ran" by the absence of that frame.
- **Low:** `x-api-key`/plural/compound secret names and Anthropic/OpenRouter/Fireworks/HF key shapes are redacted (a leak the first T05 version had); a tab's non-engine log line closes that tab's open delta run first; the tool-start map is capped; a deleted file is no longer "changed" in the Bash note; the browser rung skips test/spec/stories files and `.htm/.sass/.less` count as code for the runtime-evidence floor; the self-check keeps the LATEST six symptoms; `formatElapsed` clamps at 0 (the engine clock can sit ms ahead of the page's); `.stream` uses `overflow-wrap: break-word`.

**Left open (by design or for the owner):** a glob allow rule such as `Bash(npm *)` also covers `npm …; pkill node` (same semantics as the deletion guard's explicit-rule override); an "always allow" on a kill card records the exact command, which a later identical compound command's deletion guard also accepts; a reminder queued just before an interrupted round rides the next user message (true of every reminder); models applied in other tabs stay selectable in the shared picker until the next catalog; the browser rung can cost one round trip on an Ink `.tsx` or when a failing `npx playwright` run counts as evidence.

**Gates:** build exit 0; `typecheck:tests` clean; `npm test` **641 registered, 533 passed, 4 failed, 104 withheld** (69 ui, 27 llm, 8 artifact), 123 s — the 4 reds: `system-prompt-is-pinned` (1) and `tool-wire-contract-is-pinned` (2), failing BY DESIGN until the owner reviews and runs `npm run approve` (T03/T12 Bash bullets, T11 Edit bullet + working-method bullet), and `import-graph · an-edited-file-is-re-extracted…`, the known mtime-tick flake ("while the edited one's moved"; passes 8/8 alone; no knowledge code changed); `npm run test:ui` **69 passed, 0 failed** (57 at baseline + 12 new), 235 s; `smoke` exit 0; `test:llm` not run as a whole — only the new T10 real-model test, once (passed).

**Gateway:** 13 new records (all descriptions `draft`; the owner's draft→ready click), freshly stamped. After the stamp, **88 of 180 records are stale** (21 before this session). Owner decision T00/T01 applies — listed, not reconciled. Newly stale, because their entry files changed: `addons-listing`, `allow-all-stance`, `approval-note`, `bounded-runs-stay-bounded`, `changes-panel`, `cheap-until-used`, `circular-check-floor`, `clarify-pre-layer`, `command-shape-always-allow`, `compaction`, `connection-absent-means-cleared`, `connections-live-in-one-place`, `context-accounting`, `cost-estimate`, `create-addon-wizard`, `deletion-guard`, `deletion-scope-split`, `discoverable-in-the-palette`, `every-per-workspace-action-names-its-tab`, `file-freshness`, `fully-local-assets`, `honest-gap-outranks-a-manufactured-green`, `images-go-to-a-second-model-never-to-the-coding-one`, `interactive-turns-run-uncapped`, `interrupt`, `live-deliberation-counter`, `mid-run-steering-both-stances`, `mirror-reasoning-efforts`, `mirror-theme-names`, `mirror-token-algebra`, `name-invocation`, `one-token-algebra`, `openai-compatible-is-negotiated-not-assumed`, `permission-stances`, `pre-turn-snapshot`, `prompt-contract`, `protected-state-dir`, `provider-usage-normalization`, `reasoning-effort-clamp`, `resume`, `runtime-evidence-floor`, `self-check-sharpening`, `self-verify-rung`, `session-list`, `session-meter`, `session-report`, `setting-timing`, `slash-command-input-guard`, `slash-registry-single-source`, `stall-detector`, `subagent-spawn`, `system-prompt-assembly`, `system-prompt-is-pinned`, `the-openai-compatible-provider-can-actually-carry-an-image`, `tool-bash`, `tool-bash-background`, `tool-bash-cwd`, `tool-edit`, `tool-monitor`, `tool-registry-contract`, `tool-wire-contract-is-pinned`, `transcript`, `tui-protocol-parity`, `turn-loop`, `usage-accounting`, `usage-normalization`, `wire-round-trip`.
