...
function renderPaneNowLine(ts) {
  ...
  const tokensEl = el.querySelector(".pane-now-tokens");
  ...
  // This tab's own D(t): what ITS turn has generated so far. Each console counts
  // its own work — the panes never share a figure.
  if (tokensEl) {
...
function paneFor(id, ts) {
  if (!ts.paneEl) {
    ...
    const nowTokens = document.createElement("span");
    nowTokens.className = "pane-now-tokens hidden";
    nowTokens.title = "Output tokens generated by this turn";
    const nowTimer = document.createElement("span");
    nowTimer.className = "pane-now-timer";
    nowEl.append(nowSpin, nowText, nowTokens, nowTimer);
 ...
 * TabState has already spoken to it, which is why lastSentSafety moves too. */
function syncChromeSafetyFromFocusedTab() {
...
function syncTabOverdrive(ts) {
  ...
  ts.paneEl.classList.toggle("overdrive", on); // this screen's stance, for readers of the DOM
  // The hot line goes on the MESSAGE BOX, exactly as it does in a single
  // console — the pane's own border already means focus / needs-approval.
  const input = ts.paneEl.querySelector(".pane-input");
  if (input) input.classList.toggle("overdrive", on);
...
/** Pane button click: flip this workspace's overdrive and tell its engine.
 * Engaging plays the sweep inside THIS pane — the single console's cinematic,
 * scoped to the screen that engaged it. */
function toggleTabOverdrive(tabId) {
  const ts = tabs.get(tabId);
  if (!ts) return;
  const next = !(ts.overdrive === true);
  setTabOverdrive(tabId, next, true);
...
function applyLayout() {
  ...
  // The top-bar context total belongs to the tiled layout (the composer's own
  // counter covers the single console), so it appears and disappears with it.
...
function repaintChromeFromFocusedTab() {
  ...
  syncChromeSafetyFromFocusedTab();
  renderSessions();
